...
class TestPerceptionToReasoning:
    ...
    async def test_perception_feeds_reasoning(self, perception_module, reasoning_engine, mock_environment):
        ...
        # Hypothesis should have supporting evidence
...
class TestReasoningToDecision:
    ...
    def test_different_strategies(self, reasoning_engine, decision_module):
        """Test that decision strategies work correctly."""
        state = {"knowledge": 50}
        ...
        for strategy in [DecisionStrategy.MAXIMIZE_UTILITY, DecisionStrategy.MINIMIZE_RISK, DecisionStrategy.BALANCED]:
            decision_module.strategy = strategy
            decision = decision_module.decide(
                available_actions=actions,
                goals=goals,
                world_state=state,
